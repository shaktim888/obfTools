...
- (instancetype)initWithObjectId:(PBXProjParser*)parser objId:(NSString *)targetId data:(NSDictionary *)data targetAttrs:(NSDictionary *)targetAttrs
{
    self = [self initWithObjectId:parser objId:targetId data:data];
    if (self)
    {
        self.targetAttrs = (NSMutableDictionary *)targetAttrs;
        
        NSString *productRefId = data[@"productReference"];
        self.productReference = [[PBXFileReference alloc] initWithObjectId:parser objId:productRefId data:parser.objects.data[productRefId]];
    }
    return self;
}
