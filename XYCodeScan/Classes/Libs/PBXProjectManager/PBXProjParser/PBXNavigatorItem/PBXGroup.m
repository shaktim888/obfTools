...
- (instancetype)initWithObjectId:(PBXProjParser*)parser objId:(NSString *)objId data:(NSDictionary *)data
{
    if (self = [super initWithObjectId:parser objId:objId data:data])
    {
        ...
        if (childObjIds)
        {
            ...
            {
                NSDictionary *obj = parser.objects.data[objId];
                if (obj)
                {
                    PBXNavigatorItem *item = nil;
                    if ([obj[@"isa"] isEqualToString:@"PBXFileReference"])
                    {
                        item = [[PBXFileReference alloc] initWithObjectId:parser objId:objId data:obj];
                    }
                    else if ([obj[@"isa"] isEqualToString:@"PBXGroup"])
                    {
                        item = [[PBXGroup alloc] initWithObjectId:parser objId:objId data:obj];
                    }
                    else if ([obj[@"isa"] isEqualToString:@"PBXVariantGroup"])
                    {
                        item = [[PBXVariantGroup alloc] initWithObjectId:parser objId:objId data:obj];
                    }
...
- (PBXNavigatorItem *)addChild:(PBXNavigatorItem *)navigatorItem
{
    ...
    [_parser.objects setObject:navigatorItem];
...
- (PBXVariantGroup *)addVariantGroup:(NSString *)name
{
    ...
    PBXVariantGroup *group = [[PBXVariantGroup alloc] initWithObjectId:_parser objId:[self genObjectId] data:groupInfo];
...
- (PBXNavigatorItem *)_addFileWithPath:(NSString *)path target:(nullable PBXTarget *)target sourceTree:(NSString *)sourceTree
{
    ...
    if (target)
    {
        NSMutableDictionary * filePhaseMapping = [[NSMutableDictionary alloc] init];
        if(target.sourcesBuildPhase) {
            filePhaseMapping[@"c"]              = target.sourcesBuildPhase;
            filePhaseMapping[@"cc"]             = target.sourcesBuildPhase;
            filePhaseMapping[@"cpp"]            = target.sourcesBuildPhase;
            filePhaseMapping[@"swift"]          = target.sourcesBuildPhase;
            filePhaseMapping[@"mm"]             = target.sourcesBuildPhase;
            filePhaseMapping[@"m"]              = target.sourcesBuildPhase;
            filePhaseMapping[@"applescript"]    = target.sourcesBuildPhase;
        }
        if(target.frameworkBuildPhase) {
            filePhaseMapping[@"tbd"]            = target.sourcesBuildPhase;
            filePhaseMapping[@"a"]              = target.sourcesBuildPhase;
            filePhaseMapping[@"framework"]      = target.sourcesBuildPhase;
        }
        if(target.resourceBuildPhase) {
            filePhaseMapping[@"bundle"]         = target.resourceBuildPhase;
            filePhaseMapping[@"strings"]        = target.resourceBuildPhase;
            filePhaseMapping[@"html"]           = target.resourceBuildPhase;
            filePhaseMapping[@"jpg"]            = target.resourceBuildPhase;
            filePhaseMapping[@"jpeg"]           = target.resourceBuildPhase;
            filePhaseMapping[@"png"]            = target.resourceBuildPhase;
            filePhaseMapping[@"tif"]            = target.resourceBuildPhase;
            filePhaseMapping[@"tiff"]           = target.resourceBuildPhase;
            filePhaseMapping[@"xcassets"]       = target.resourceBuildPhase;
            filePhaseMapping[@"xib"]            = target.resourceBuildPhase;
        }
        // 拿到 build phase
    ...
    if (!fileObj)
    {
        ...
        fileObj = [[PBXFileReference alloc] initWithObjectId:_parser objId:[self genObjectId] data:info];
...
- (PBXGroup *)_addGroup:(NSString *)name path:(NSString *)path sourceTree:(NSString *)sourceTree
{
    ...
    PBXGroup *group = [[PBXGroup alloc] initWithObjectId:_parser objId:[self genObjectId] data:groupInfo];
...
- (void)_removeChild:(PBXNavigatorItem *)navigatorItem
{
    ...
    if ([self.children containsObject:navigatorItem])
    {
            ...
            && ((PBXGroup *)navigatorItem).children)
        {
            ...
            {
                for (PBXTarget *target in _parser.project.targets)
                {
        ...
        [_parser.objects deleteObject:navigatorItem];
    }
}
