...
function llex.init(_z, _sourceid)
  z = _z                        -- source
  sourceid = _sourceid          -- name of source
  I = 1                         -- lexer's position in source
  ln = 1                        -- line number
  llex.tok = {}                      -- lexed token list*
  llex.seminfo = {}                  -- lexed semantic information list*
  llex.tokln = {}                    -- line numbers for messages*
...
function llex.errorline(s, line)
  local e = error or base.error
  e(string.format("%s:%d: %s", llex.chunkid(), line or ln, s))
end
local errorline = llex.errorline
